...
from topic_analysis import remove_nulls, remove_stopwords, remove_urls, topicwizard_plots
...
if display_stat == "Scorecard" and match_id != 0:
    try:
        with st.spinner("Fetching Scorecard..."):
            if reddit == "":
                reddit, post_page = fetch_reddit_post(desc)
    ...
    try:
        ...
        if len(all_comments) != 0:
            ...
            with st.spinner("Analysing clusters..."):
                ...
                topical_map, topical_barchart, topical_word_cloud = topicwizard_plots(cleaned_input_text)
            try:
            ...
            #         components.html(html_content, height=700)
                ...
                tab1, tab2, tab3 = st.tabs(["Topical Map", "Bar chart", "Word Cloud"])
                with tab1:
                    st.plotly_chart(topical_map)
                with tab2:
                    st.plotly_chart(topical_barchart)
                with tab3:
                    st.plotly_chart(topical_word_cloud)
    ...
    try:
        ...
        with st.spinner("Visualizing..."):
                ...
                with tab1:
                    ...
                    st.plotly_chart(hist_plot, theme=None)
                with tab2:
                    st.subheader("Public emotions on Reddit")
                    st.plotly_chart(pie_plot, theme=None)
